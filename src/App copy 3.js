...
// robust mapper: takes an API patient object and returns the exact fields used by the form
function mapApiPatientToForm(apiPatient) {
  if (!apiPatient) return { name: "", mrn: "", birthDate: "", gender: "", phone: "" };

  // Name handling: support several possible shapes from API
  let name = "";
  if (apiPatient.name && typeof apiPatient.name === "string") {
    name = apiPatient.name;
  } else if (apiPatient.fullName) {
    name = apiPatient.fullName;
  } else if (Array.isArray(apiPatient.name) && apiPatient.name.length) {
    // FHIR style: name[0].given / name[0].family
    const n = apiPatient.name[0];
    ...
    // fallback to first/last style
    const first = apiPatient.firstName || apiPatient.givenName || apiPatient.given || "";
    const last = apiPatient.lastName || apiPatient.familyName || apiPatient.family || "";
  ...
  // BirthDate normalization: accept many formats, convert to YYYY-MM-DD if possible
  let birthDate = apiPatient.dob || apiPatient.birthDate || apiPatient.dateOfBirth || "";
  if (birthDate) {
    // If in ISO datetime, take the date part
    if (birthDate.includes("T")) birthDate = birthDate.split("T")[0];
    // If D/M/Y or similar, try to normalize simple dd-mm-yyyy or dd/mm/yyyy -> yyyy-mm-dd
    const dmy = birthDate.match(/^(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{2,4})$/);
    if (dmy) {
      let [_, dd, mm, yyyy] = dmy;
      if (yyyy.length === 2) yyyy = "19" + yyyy; // simple heuristic; adjust if needed
      birthDate = `${yyyy.padStart(4, "0")}-${mm.padStart(2, "0")}-${dd.padStart(2, "0")}`;
    }
  }

  // Phone / mobile: support telecom array or direct fields
  let phone = "";
  if (apiPatient.mobile) phone = apiPatient.mobile;
  else if (apiPatient.phone) phone = apiPatient.phone;
  else if (Array.isArray(apiPatient.telecom) && apiPatient.telecom.length) {
    const ph = apiPatient.telecom.find((t) => t.system === "phone" || t.system === "tel");
    if (ph) phone = ph.value || "";
  }

  // gender
  const gender = (apiPatient.gender || apiPatient.sex || "").toLowerCase();

  // Bring in any other fields you want available later (kept intact)
  const extras = { raw: apiPatient };
...
export default function AppConsult() {
  ...
  // handle selection from the patient dropdown (id could be abha_ref / mrn / id / user_id)
  async function handlePatientSelect(id) {
    ...
    if (patientCacheRef.current.has(id)) {
      const cached = patientCacheRef.current.get(id);
      setPatient(mapApiPatientToForm(cached));
      return;
    }

    // Try to find in the loaded list (patientListAll) first
    const found = patientListAll.find((p) => {
      const keys = [p.user_id, p.id, p.mrn, p.abha_ref, p.mobile, p.phone].map((v) => (v ? String(v) : ""));
      return keys.includes(id) || (p.name && p.name.toLowerCase().includes(String(id).toLowerCase()));
    });
    ...
    // If not found locally, attempt to fetch detail file from /patients/{id}.json (optional)
    try {
      setPatientLoading(true);
      ...
      console.warn("patient detail fetch failed:", err);
    } finally {
      setPatientLoading(false);
    }

    // Last resort: try to match by id substring in patientListAll and pick first match
    const fuzzy = patientListAll.find(
      (p) =>
        (p.name && p.name.toLowerCase().includes(String(id).toLowerCase())) ||
        (p.mrn && p.mrn.includes(id)) ||
        (p.mobile && p.mobile.includes(id))
    );
    if (fuzzy) {
      patientCacheRef.current.set(id, fuzzy);
      setPatient(mapApiPatientToForm(fuzzy));
      return;
    }

    // If nothing found, clear the patient but keep the id selected
    setPatient({ name: "", mrn: id, birthDate: "", gender: "", phone: "" });
  }
